...
async function GET(req) {
    try {
        ...
        const data = queues.map((q)=>{
            ...
            const groups = q.transactions.map((t)=>(t.groupItemMaster ?? t.descriptionItemPrice ?? "").toLowerCase());
            ...
            const serviceNames = q.transactions.map((t)=>t.descriptionItemPrice).filter(Boolean).slice(0, 3);
            const description = serviceNames.length > 0 ? serviceNames.join(", ") + (q.transactions.length > 3 ? ` +${q.transactions.length - 3} more` : "") : q.qFullName ?? "Visit";
            return {
                id: Number(q.id),
...
//# sourceMappingURL=%5Broot-of-the-server%5D__7dd00861._.js.map